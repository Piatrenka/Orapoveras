...
(select sum(nvl(m.SCORE1, 0)) from vmatches m where m.teamid1 = t.teamid) + (select sum(nvl(m.SCORE2, 0)) from vmatches m where m.teamid2 = t.teamid) Scored,
(select sum(nvl(m.SCORE2, 0)) from vmatches m where m.teamid1 = t.teamid) + (select sum(nvl(m.SCORE1, 0)) from vmatches m where m.teamid2 = t.teamid) Missed,

(select sum(nvl(m.SCORE1, 0)) from vmatches m where m.teamid1 = t.teamid) + (select sum(nvl(m.SCORE2, 0)) from vmatches m where m.teamid2 = t.teamid) -
((select sum(nvl(m.SCORE2, 0)) from vmatches m where m.teamid1 = t.teamid) + (select sum(nvl(m.SCORE1, 0)) from vmatches m where m.teamid2 = t.teamid)) Netto,
